...
template<int nrows, int ncols> struct mat {
    ...
    double det() const { return dt<nrows>::det(*this); }
    ...
    mat<nrows, ncols> invert_transpose() const {
        mat<nrows, ncols> adj;
        for (int i = nrows; i--; )
            for (int j = ncols; j--; adj[i][j] = cofactor(i, j));
